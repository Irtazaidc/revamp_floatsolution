...
export class SubheaderService implements OnDestroy {
  ...
  private notificationResp$ = new BehaviorSubject({});

  startConnection = () => {
    console.log('Initializing Hub Connection...');
    this.hubConnection = new signalR.HubConnectionBuilder()
      // .withUrl(environment.RealtimeMetacubesUrl + 'PushRiderNotifications/SendSpecificNotiToSpecificDevice', {
      .withUrl('http://localhost:51682/api/booking-chat', {
    ...
    this.hubConnection
      .start()
      .then(() => {
        console.log('Hub Connection Established Successfully!');
        this.askServerListener(); // Set up listeners after successful connection
      })
      .catch((err) => {
        console.error('Error while starting connection:', err.message);
  ...
  askServer() {
    console.log('Calling askServer...');
    this.hubConnection
      ?.invoke('askServer', 'hey')
      .then(() => {
        console.log('askServer invoked successfully.');
      })
      .catch((err) => {
        console.error('Error while invoking askServer:', err.message);
      });
  }

  askServerListener() {
    console.log('Setting up askServerListener...');
    try {
      this.hubConnection?.on('askServerResponse', (response) => {
        console.log('askServerResponse received:', response);
        this.notificationResp$.next(response || {});
      });
    } catch (err) {
      console.error('Error in askServerListener:', err.message);
    }
  }
  setDefaultSubheader() {
  ...
  private getBreadcrumbsAndTitle(menuId, pathName): SubheaderModel {
    ...
    if (menuId == 'kt_aside_menu') {
      menu = document.querySelector('[id^="kt_aside_tab"].show.active [id="kt_aside_menu"]');
    }
    if (!menu) {
      return result;
    }
