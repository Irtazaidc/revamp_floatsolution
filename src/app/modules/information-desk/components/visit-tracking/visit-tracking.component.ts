...
export class VisitTrackingComponent implements OnInit {
  ...
  selectVisit(visit) {
    ...
    if (visit && visit.VisitID || visit.VisitId) {
      this.getVisitDetails(visit.VisitID || visit.VisitId);
    } else {
  ...
  cancellationTPChecked() {
    ...
    setTimeout(() => {
      ...
      console.log('amountToRefund amountToRefund amountToRefund ', amountToRefund);
  ...
  saveCancellationPopup(cancellationStatusId) {
    ...
    */
    ...
    console.log('======================================================> ', dataToPost, fbrRequestData);
  ...
  openInvoice(visit) {
    console.log('Visit row___________', visit, '  visitid: ', visit.VisitId);//return;
  ...
  openMOHistoryReport(row) {
    console.log("Button for report view working:::", row);

    let visitNo = row?.VisitNo || this.selectedVisit?.VisitID || this.selectedVisit?.VisitId;

    if (!visitNo) {
        console.error("VisitNo or VisitID is missing, unable to proceed.");
        return;
    }

    visitNo = visitNo.toString().replace(/-/g, "");  // Ensure it's formatted correctly

    const url = environment.patientReportsPortalUrl + 'mo-consent?p=' + 
        btoa(JSON.stringify({ VisitID: Number(visitNo), TPID: row.TPId }));
