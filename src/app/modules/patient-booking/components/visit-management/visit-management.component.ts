...
export class VisitManagementComponent implements OnInit {
  ...
  constructor(
    ...
    private postexService: PostexService
  ) { }
  ...
  getVisitDetails(visitID) {
    ...
    if (params.VisitId) {
      this.spinner.show();
      this.visitService.getVisitDetails(params).subscribe(
        (res: any) => {
          this.scrollToBottom();
          this.spinner.hide();
          console.log(res);
          if (res && res.StatusCode == 200 && res.PayLoadDS) {
            ...
            this.visitDetails = {
              pateintInfo: res.PayLoadDS.Table.length
                ? res.PayLoadDS.Table[0]
                : null,
              visitInfo: res.PayLoadDS.Table1.length
                ? res.PayLoadDS.Table1[0]
                : null,
  ...
  patientVisitEvent(event) {
    console.log("visit Information ", event);
  ...
  openCancellationForm() {
    ...
    //   return;
    // }
    this.tpStatusForCancellation = [1, 2, 6];
    this.tpStatusForRequestForCancellation = [3, 4, 5, 7, 8];
    ...
    if (this.removeCancellationLimit) {
      this.tpStatusForCancellation = [1, 2, 6];
      this.tpStatusForRequestForCancellation = [3, 4, 5, 7, 8, 9, 10, 11, 12, 13, 14];
    }
  ...
  saveCancellationPopup(cancellationStatusId) {
    ...
    */
    ...
    this.visitService.cancelVisit(dataToPost).subscribe(
      (res: any) => {
  ...
  paymentModeChangedEvent(event, formName) {
    ...
    this.selectedPaymentMOdeId = event.target.value;
    ...
    if (event.target.value == 8) {
      this.setJazzCashFieldsValidators(true, _formRef);
    } 
    else {
      this.setCreditCardFieldsValidators(false, _formRef);
    }
  ...
  setJazzCashFieldsValidators(required, form) {
    if (required) {
      form.controls["CCNo"].setValidators([Validators.required]);
      form.controls["InstOwner"].setValidators([Validators.required]);
      // form.controls["CCTNo"].setValidators([Validators.required]);
  ...
  saveReceiveInstallmentPopup() {
    ...
    if (formValues.receivingAmount > formValues.balance) {
     ...
     const hasMode6 = formValues.paymentMode == 6 ? true : false;
  ...
  //#OnlineBanking Region
   ...
   qrString = '';
    ...
    printReceipt() {
      ...
      const patientName =
        this.patientBasicInfo.Saulutation + ' ' + this.patientBasicInfo.FirstName + ' ' +
        (this.patientBasicInfo.LastName ? this.patientBasicInfo.LastName : '');
  
      const patientCell = this.patientBasicInfo.MobileNO;
    ...
    GetOnlinePaymentReferenceforQRCode(): Promise<any> {
      return new Promise((resolve, reject) => {
        let param = {
          ...
          BranchID: this.loggedInUser.locationid,
          PatientID: this.visitDetails.pateintInfo.PatientId || null,
          PatientName:  this.patientBasicInfo.Saulutation + ' ' + this.patientBasicInfo.FirstName + ' ' + (this.patientBasicInfo.LastName ? this.patientBasicInfo.LastName : ''),
          PatientCellNo: this.patientBasicInfo?.MobileNO,
          CreatedBy: this.loggedInUser.userid,
   ...
   generateRRN(): string {
    ...
    getCardValue(event) {
    console.log("🚀 ~ Patient CardValue ~ event:", event);
   this.patientBasicInfo = {
      Saulutation: event.SalutationTitle,
      FirstName: event.FirstName,
      LastName: event.LastName,
      MobileNO: event.MobileNO,
      Location: event.RegistrationLocation,
    };
  }
    // ------------------------------------------------------------------
