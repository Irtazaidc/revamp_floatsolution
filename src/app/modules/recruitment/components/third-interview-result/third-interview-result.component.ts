...
@Component({
  ...
  selector: 'app-third-interview-result',
  templateUrl: './third-interview-result.component.html',
  styleUrls: ['./third-interview-result.component.scss']
})
export class ThirdInterviewResultComponent implements OnInit {
  ...
  isSpinnerFinalized: boolean = true;//Hide Loader
  ...
  changeJobApplicantRequestStatus(statusID) {
    ...
    if (statusID == 12) {
      alertMessage = 'Applicant has been Finalized successfully';
    } else if (statusID == 3) {
  ...
  applicantProcess() {
    ...
    this.spinner.show(this.spinnerRefs.applicantListSection);
    let jobListChecked = this.ApplicantResult.filter(a => a.checked);
  ...
  getApplicantStatus() {
    ...
    this.recruitment.getApplicantStatus().subscribe((res: any) => {
      ...
      resApplicanttStatus = resApplicanttStatus.filter(a => (a.ApplicantStatusID == 3 || a.ApplicantStatusID == 12));
