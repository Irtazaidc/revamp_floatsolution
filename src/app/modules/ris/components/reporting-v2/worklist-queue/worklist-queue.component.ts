...
export class WorklistQueueComponent implements OnInit, OnDestroy, OnChanges, AfterViewInit {
  ...
  private fetchData(): void {
    ...
    if (!this.paramsValuesForWorkList.visitID && this.paramsValuesForWorkList.dateFrom && this.paramsValuesForWorkList.dateTo) {
      ...
      if (this.screenIdentity === 'reporting-v2') {
  ...
  verifyUserBiomateric(row, index) {
    ...
    } else if (this.screenIdentity == 'reporting-v2') {
      this.TPId = row.TPId;
      this.VisitID = row.VisitNo.replaceAll("-", "");
      this.getRISWorklistForReportingRow(Number(this.VisitID), this.TPId);
      setTimeout(() => {
        if (
          (this.active === 3 || this.active === 7)  &&  row.StatusId < 8 &&
          ((!this.isVisitIDSearch && this.DSByEmpID !== this.InitByEmpID) || (this.isVisitIDSearch && this.DSByEmpID == this.loggedInUser.empid && this.DSByEmpID !== this.InitByEmpID))
        ) {
          ...
          this.sharedService
            .getData(API_ROUTES.GET_EMPLOYEE_NAME_LOCATION, objParam)
            .subscribe(
              (data: any) => {
                const response = data?.PayLoad;
                const initDoctorName = response[0]?.EmpNameLocation || 'Assigned Doctor';
                Swal.fire({
                  title: 'Initial Report Pending',
                  width: '38em',
                  html: `
                  <div class="row no-gutters align-items-start mt-2"
                      style="
                    ...
                    <div class="col-auto pr-2">
                      <img src="./assets/images/brand/info-icon-warning.png"
                          alt="Warning Icon"
                    ...
                    <div class="col" style="padding-left: 0;">
                      <small class="d-block"
                            style="color: #636363;
                                    ...
                                    line-height: 1.4;">
                        Study assigned to 
                        <strong>${initDoctorName}</strong>
                        for Initial Reporting.
                        <br>
                        Digital Sign (DS) can be completed only after the Initial report is finalized.
                      </small>
                    </div>
                  </div>
                `,
        ...
        this.isCompareStudy = row.isCompareStudy;
        this.RISRequestAIID = row.RISRequestAIID;
        this.visitInfo = { tpId: this.TPId, visitID: this.VisitID, patientID: this.PatientId, phoneNumber: this.PatientPhoneNumber }
        this.getVitals();
  ...
  isVisitIDSearch = false;
  getRisworkList(val) {
    this.isVisitIDSearch = val.visitID ? true : false;
    this.screenIdentity = this.route.routeConfig.path;
    ...
    if ((val.dateFrom && val.dateTo) || val.visitID) {
      ...
      const params = {
        ...
        SubSectionIDs: (val.filterBy === 5 && (!val.subSectionIDs || val.subSectionIDs == "") && this.screenIdentity == 'reporting-v2') ? val.subSectionIDsAll.join(",") : (val.subSectionIDs ? val.subSectionIDs.join(",") : null),
        AssignerFilter: val.AssignemntFilter,
        UserID: this.loggedInUser.userid || -99 // -99 incase of null
      }
      this.worklistSrv.getRISWorklistForReporting(params).subscribe((resp: any) => {
        ...
        if (resp && resp.PayLoad && resp.PayLoad.length && resp.StatusCode == 200) {
          ...
          this.risWorkist = dataset.map(a => ({
            ...
            isMetal: a.isMetal,
            isDoctorFeedback: a.isDoctorFeedback,
            ...
            isCompareStudy: a.isCompareStudy
          }));
  ...
  countInit = 0;
  countDS = 0;
  ////////////////////// begin:: Summary main method //////////////////
  getRISWorkListSummary(val) {
    this.getRISWorkListSummaryMain(val)
    this.getRISWorkListSummaryUnassigned(val)
    this.getRISWorkListSummaryAddendumSecondOpinion(val)
  }
  ////////////////////// end:: Summary main method //////////////////
    ...
    if ((val.dateFrom && val.dateTo) || val.visitID) {
      ...
      this.worklistSrv.getRISWorkListSummaryMain(params).subscribe((resp: any) => {
        if (resp && resp.PayLoadDS && resp.StatusCode == 200) {
          ...
          // setTimeout(() => {
          //   console.log("main all:", summaryCounter)
          //   console.log("dswl:", dswl)
          //   console.log("this.loggedInUser.empid:", this.loggedInUser.empid)
          // }, 300);
  ...
  getRISWorkListSummaryAddendumSecondOpinion(val) {
    if ((val.dateFrom && val.dateTo) || val.visitID) {
      ...
      this.worklistSrv.getRISWorkListSummaryAddendumSecondOpinion(params).subscribe((resp: any) => {
  ...
  getTestSummaryInfo(activeTab) {
    switch (activeTab) {
      ...
      // }
      case 8: {
        this.testSummaryInfoHeader = this.screenIdentity === 'reporting-v2' ? "Case Studies" : "Pended Worklist";
        break
      }
      case 9: {
  ...
  openMOHistory(row) {
    console.log("mo history row is ______", row)
    this.isMetal = row.isMetal;
    this.isTechDisclaimer = false;
    // console.log("mo-row", row);
    this.TPId = row.TPId;
  ...
  closeReportingModal_() {
    ...
    this.sharedService.insertUpdateData(API_ROUTES.UPDATE_RADIO_REPORT_LOCK, objParam).subscribe((data: any) => {
      if (data.StatusCode == 200) {
        console.warn("Study has been unlocked");
      } else {
        console.warn('Error occured while Unlock the study!')
      }
    }, (err) => {
      console.log(err);
      console.warn('Connection with database is not made,  Please contact System Support !');
    })
