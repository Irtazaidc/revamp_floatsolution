...
@Component({
  ...
  selector: 'app-tech-audit-form',
  templateUrl: './tech-audit-form.component.html',
  styleUrls: ['./tech-audit-form.component.scss']
})
export class TechAuditFormComponent implements OnInit {
    ...
    this.sharedService.getData(API_ROUTES.GET_TECHNOLOGIST_VISIT_TP_AUDIT_BY_ID, objParam).subscribe((resp: any) => {
      if (resp.PayLoadDS && resp.StatusCode == 200) {
        ...
        if(this.existingAuditRow.length){
          ...
          this.RecommendedFine = objRow.RecommendedFine;
          ...
          this.feedbackBy = objRow.ManagerName;
  ...
  ManagerRemarks	= null;
  ApprovedFine = 0;
  insertUpdateTechnologistVisitTPAudit() {
    ...
    if (!this.AuditQAIDs.length) {
      ...
      let objParam = {
        ...
        TPID: this.TPID,
        AuditQAIDs: this.AuditQAIDs ? this.AuditQAIDs.join(",") : null,
        AuditStatusID: this.saveWithShare?6:4, //IAuditSatus Table 6 is for Tech Audit Saved with Shared else only saved wich is 4 for tech audit saved
        Recommendation: this.Recommendations,
        Remarks: this.Remarks,
        Rating: this.AuditRating,
        RecommendedFine: this.RecommendedFine,
        CreatedBy: this.loggedInUser.userid || -99,
        isReAudit: this.isReAudit
      }
      // console.log("Form values_________", objParam, this.AuditQAIDs);//return;
      this.disabledButton = true;
      this.isSpinner = false;
      this.spinner.show(this.spinnerRefs.formSection);
      this.sharedService.insertUpdateData(API_ROUTES.INSERT_UPDATE_TECHNOLOGIST_VISIT_TP_AUDIT, objParam).subscribe((resp: any) => {
