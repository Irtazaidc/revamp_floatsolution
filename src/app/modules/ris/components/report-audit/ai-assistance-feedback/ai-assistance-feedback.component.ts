...
@Component({
  ...
  selector: 'app-ai-assistance-feedback',
...
export class AiAssistanceFeedbackComponent implements OnInit {
  ...
  doctorAIFeedbackAuditPopupRef: NgbModalRef;
  @ViewChild('doctorAIAssistFeedbackAudit') doctorAIAssistFeedbackAudit;
  public RISParams = {
    dateFrom: [null, ''],
    dateTo: [null, ''],
    visitID: [
  ...
  getRISWorkList() {
    ...
    this.sharedService.getData(API_ROUTES.AI_ASSISTANCE_FEEDBACK, params).subscribe((resp: any) => {
  ...
  AuditorRadiologistRating = null;
  AuditorRadiologistRemarks = null;
  setRatingValue(rating) {
    this.clickSubmitAIRequestFeedback = false;
    this.AuditorRadiologistRating = rating;
  }
  VisitId = null;
  TPID = null;
  PatientID = null;
  ...
  openFeedBackAuditModal(row, i) {
    ...
    this.sharedService.getData(API_ROUTES.GET_EXISTINCE_AI_ASSISTANCE_FEEDBACK, params).subscribe((resp: any) => {
      if (resp && resp.PayLoad && resp.PayLoad.length && resp.StatusCode == 200) {
        ...
        if (this.existingFeedback.length) {
          this.AuditorRadiologistRating = this.existingFeedback[0].AuditorRadiologistRating;
          this.AuditorRadiologistRemarks = this.existingFeedback[0].AuditorRadiologistRemarks;
          ...
          this.RadiologistName = this.existingFeedback[0].RadiologistName;
        }
      }
    }, (err) => {
      console.log("Err", err)
    ...
    setTimeout(() => {
      ...
      this.getRISAIAssistanceRequestByVIsitIDTPIDDoubleClick(row, i);
      this.doctorAIFeedbackAuditPopupRef = this.appPopupService.openModal(this.doctorAIAssistFeedbackAudit, { backdrop: 'static', size: 'md' });
    }, 500);

  }
  openFeedBackAuditModaldblClick(row, i) {
    this.rowIndex = i;
    this.VisitId = row.VisitID;
    this.TPID = row.TPID;
    this.PatientID = row.PatientID;
  ...
  updateAIAssistenceFeedBackAudit() {
    ...
    this.sharedService.insertUpdateData(API_ROUTES.AI_ASSISTANCE_DOCTOR_FEEDBACK_AUDIT, dataObj).subscribe((data: any) => {
      if (data.StatusCode == 200) {
        // this.toastr.success(data.Message);
        this.toastr.success("Your feedback for the AI assistant has been saved.");
        this.disabledButtonDoctorAIFeedback = false;
        this.isSpinnerDoctorAIFeedback = true;
        this.doctorAIFeedbackAuditPopupRef.close();
      }
      else {
  ...
  disabledButtonAIImage = false;
  isSpinnerAIImage = true;
  ...
  getAndOpenDicomImageByButton(row, index) {
    ...
    if (this.storedDicomData.imagePath) {
      ...
      this.toastr.info("Opened DICOM image");
      return;
    }
  ...
  getRISAIAssistanceRequestByVIsitIDTPIDDropDown(row, i) {
    ...
    this.sharedService.getData(API_ROUTES.GET_RIS_AI_ASSISTANCE_REQUEST_BY_VISIT_ID_TPID, params).subscribe((res: any) => {
      ...
      console.log(err);
      this.toastr.error('Error loading AI assistance request feedback');
    });
  }
