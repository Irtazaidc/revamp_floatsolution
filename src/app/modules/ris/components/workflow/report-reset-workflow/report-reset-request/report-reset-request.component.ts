...
@Component({
  ...
  selector: 'app-report-reset-request',
  templateUrl: './report-reset-request.component.html',
  styleUrls: ['./report-reset-request.component.scss']
})
export class ReportResetRequestComponent implements OnInit {
  ...
  selectVisit(visit) {
    ...
    if (visit && visit.VisitID) {
      this.getVisitTestsByVisitIDStatusID(visit.VisitID, 9);
    } else {
  ...
  updateVisitTestResetStatus() {
    ...
    if (!checkedTests.length) {
      ...
      this.sharedService.insertUpdateData(API_ROUTES.UPDATE_VISIT_TEST_RESET_STATUS, formData).subscribe((data: any) => {
        ...
        this.getVisitTestsByVisitIDStatusID(this.VisitID, 9);
