...
@Component({
  ...
  selector: 'app-ris-worklist',
  templateUrl: './ris-worklist.component.html',
  styleUrls: ['./ris-worklist.component.scss']
})

export class RisWorklistComponent implements OnInit {
  ...
  confirmationPopoverConfigCloseModalReporting = {
    placements: ['top', 'left', 'right', 'bottom'],
    popoverTitle: '<span class="text-danger">Are you <strong>sure</strong> want to close? <span>', // 'Are you sure?',
    popoverMessage: 'All <strong>Unsaved data</strong> will be <strong>Discarded</strong> and report will be unlocked',
    confirmText: 'Yes <i class="fa fa-check"></i>',
    cancelText: 'No <i class="fa fa-times"></i>',
    confirmClicked: false,
    cancelClicked: false,
  ...
  private fetchData(): void {
    ...
    if (!this.paramsValuesForWorkList.visitID && this.paramsValuesForWorkList.dateFrom && this.paramsValuesForWorkList.dateTo) {
      ...
      if (this.screenIdentity === 'reporting-worklist') {
        this.getRISWorkListSummaryOutsanding(this.paramsValuesForWorkList);
      }
    }
  }
  ngOnInit(): void {
    ...
    this.currentDateTime = moment().format('YYYY-MM-DDTHH:mm:ss');
    ...
    if (this.screenIdentity == 'queue-management')
    ...
    if (
      this.screenIdentity === 'queue-management'
      || this.screenIdentity === 'assign-bulk-test'
      || this.screenIdentity === 'queue-manager'
      || this.screenIdentity === 'bulk-queue-manager') {
      this.getRisworkList(this.paramsValuesForWorkList);
    }
    // this.getRisworkList(this.paramsValuesForWorkList);
        ...
        setTimeout(() => {
          ...
          if (resp && resp.userIdentity && resp.userIdentity != 0) {
            if (resp.userIdentity == -99) {
              ...
              this.isMetal = data.value.isMetal;
    ...
    setTimeout(() => {
      if (!this.multiAppConnectionStatus && this.screenIdentity != 'reporting-worklist') {
  ...
  ngOnChanges(changes: SimpleChanges) {
    ...
    if (this.screenIdentityngOnchages != 'queue-management' && this.screenIdentityngOnchages != 'assign-bulk-test') {
      // setTimeout(() => {
      this.getRisworkList(this.paramsValuesForWorkList);
      // }, 200);
    }
  ...
  verifyUserBiomateric(row, index) {
    ...
    if (this.screenIdentity == 'mo-worklist') {
    ...
    } else if (this.screenIdentity == 'queue-management') {
      this.isTechDisclaimer = false;
      if (row.MOBy)
        ...
        setTimeout(() => {
          if (this.StatusId >= 7 || (this.StatusId == -1) || (this.StatusId == -2)) {
            ...
            if (this.count == 1) {
              ...
              this.toastr.warning("Please verify your thumb for Visit: " + this.VisitIDWithDashes + ", TPCode: " + this.TPCode + ", Patient: " + this.PatientName, "instance already opened");
            }
          }
        }, 300);
        //Check for already initialized study
      ...
      this.tempDSDateTime = row.tempDSDateTime;
      this.EditExpiryTime = row.EditExpiryTime;
      this.ReportDelayTime = row.ReportDelayTime;
      this.EmpID = row.EmpId
      this.IsAIAssistEnable = row.IsAIAssistEnable
      ...
      setTimeout(() => {
        ...
        this.reportingModalRef = this.appPopupService.openModal(this.reportingModal, { backdrop: 'static', size: 'fss' });
      }, 200);
    }
  }
  ...
  spinnerRefs = {
    ...
    searchDoctorShareTable: 'searchDoctorShareTable'
  }
  _object = Object;
  showAllTab = false;
  ...
  getRisworkList(val) {
    console.log("Val: ________________",val)
    this.screenIdentity = this.route.routeConfig.path;
    ...
    if ((val.dateFrom && val.dateTo) || val.visitID) {
      ...
      let params = {
        ...
        SubSectionIDs: (val.filterBy === 5 && (!val.subSectionIDs || val.subSectionIDs == "") && this.screenIdentity == 'reporting-worklist') ? val.subSectionIDsAll.join(",") : (val.subSectionIDs ? val.subSectionIDs.join(",") : null),
      ...
      this.worklistSrv.getRISWorklist(params).subscribe((resp: any) => {
        ...
        if (resp && resp.PayLoad && resp.PayLoad.length && resp.StatusCode == 200) {
          ...
          this.risWorkist = dataset.map(a => ({
            ...
            ReportDelayTime: a.ReportDelayTime,
            "EditExpiryTime": this.getExpiryTimeFromDateTime(a.tempDSDateTime, a.ReportDelayTime),
            tempDSDateTime: a.tempDSDateTime ? this.removeIntervals(a.tempDSDateTime) : null,
            isMetal: a.isMetal,
            RadiologistID: a.RadiologistID,
            IsAIAssistEnable: a.IsAIAssistEnable,
            LocId: a.LocId,
            RISRequestAIID: a.RISRequestAIID,
          ...
          this.filterResults();
          console.log("this.risWorkist_______________", this.risWorkist)
  ...
  getExpiryTimeFromDateTime(deliveryDate: string, minutesToAdd: number): string {
    if (deliveryDate && minutesToAdd != null) {
      const updatedTime = moment(deliveryDate).add(minutesToAdd, 'minutes');
      return updatedTime.format('h:mm:ss A');
    } else {
      return "--";
    }
  }
  ...
  getRISWorkListSummary(val) {
    // console.log("getRISWorkListSummary:",val)
    if ((val.dateFrom && val.dateTo) || val.visitID) {
      let params = {
        VisitID: val.visitID ? val.visitID.replaceAll("-", '') : null,
        // BranchIDs: val.branch ? val.branch.join(",") : null,
        BranchIDs: (this.screenIdentity === 'queue-management' || this.screenIdentity === 'assign-bulk-test' || this.screenIdentity === 'queue-manager' || this.screenIdentity == 'bulk-queue-manager' || this.screenIdentity == 'mo-worklist' || this.screenIdentity == 'tech-worklist' || this.screenIdentity == 'reporting-worklist') && (!val.branch || !val.branch.length)
          ? val.branchAll.join(",")
        ...
        // SubSectionIDs: (val.filterBy === 5 && (!val.subSectionIDs || val.subSectionIDs == "") && this.screenIdentity == 'reporting-worklist') ? val.subSectionIDsAll.join(",") : (val.subSectionIDs ? val.subSectionIDs.join(",") : null),
        SubSectionIDs: (
          (val.filterBy === 1 || val.filterBy === 2 || val.filterBy === 3 || val.filterBy === 5 || val.filterBy === 8 || val.filterBy === 12)
          && (!val.subSectionIDs || val.subSectionIDs == "")
          && (this.screenIdentity === 'queue-management' || this.screenIdentity === 'assign-bulk-test' || this.screenIdentity == 'reporting-worklist' || this.screenIdentity == 'queue-manager' || this.screenIdentity == 'bulk-queue-manager' || this.screenIdentity == 'mo-worklist' || this.screenIdentity == 'tech-worklist')
        ) ? val.subSectionIDsAll.join(",") : (val.subSectionIDs ? val.subSectionIDs.join(",") : null),

        UserID: this.loggedInUser.userid || -99
      }
      // console.log("params areeeeeeeeeeee:",params)
      this.worklistSrv.getRISWorkListSummary(params).subscribe((resp: any) => {
        let filterBy = [4, 5, 6, 7, 8, 9, 10];
        if (filterBy.includes(parseInt(params.FilterBy))) {
          if (resp && resp.PayLoadDS && resp.StatusCode == 200) {
            ...
            let summaryAddendum = resp.PayLoadDS.Table1;
            let summaryPending = resp.PayLoadDS.Table2;
            ...
            let draftednwl = summaryCounter.filter(a => a.RISStatusID == 10 && a.StatusID == 7);
            ...
            let reportednwl = summaryCounter.filter(a => a.StatusID == 8);
            ...
            let finalwl = summaryCounter.filter(a => a.StatusID == 9 || a.StatusID == 11 || a.StatusID == 12);
          ...
          if (resp && resp.PayLoadDS && resp.StatusCode == 200) {
            ...
            let mowl = summaryCounter.filter(a => !a.RISStatusID && a.isMedicalOfficerIntervention == 1);
            this.countMOWorklist = mowl.length ? mowl[0].TestCount : 0 || 0;

            let modonewl = summaryCounter.filter(a => a.RISStatusID == 2 && a.isMedicalOfficerIntervention == 1);
            this.countMODone = modonewl.length ? modonewl[0].TestCount : 0 || 0;

            let checkinwl = summaryCounter.filter(a => a.RISStatusID == 2 || a.RISStatusID == 7 || !a.RISStatusID);
            ...
            let assignerAllwl = resp.PayLoadDS.Table1.length ? resp.PayLoadDS.Table1[0].TestCount : 0




            let assigneAssignedwl = summaryCounter.filter(a => a.RISStatusID == 8);
            ...
            let assignerUnassignedwl = summaryCounter.filter(a => [4, 5, 9].includes(a.RISStatusID)); //1, 2, 3, 4, 5, 6, 7, 9 i have made some changes because in counter MO done and checkin studies was also comming while not in listing
  ...
  getRISWorkListSummaryOutsanding(val) {
    if ((val.dateFrom && val.dateTo) || val.visitID) {
      ...
      this.worklistSrv.getRISWorkListSummaryOutsanding(params).subscribe((resp: any) => {
        if (resp && resp.PayLoadDS && resp.StatusCode == 200) {
          this.countOustanding = resp.PayLoadDS.Table.length ? resp.PayLoadDS.Table[0].OutStandingCount : 0
        }
      }, (err) => {
        console.log("Err", err)
      })
  ...
  getActive(active, risStatusID, filterBy, formHeaderBGClass, formHeaderText) {
    ...
    if (filterBy == -99) {
      if (this.screenIdentity == 'mo-worklist') {
        ...
        filterByCondition = 2;
      } else if ((this.screenIdentity == 'queue-management' || this.screenIdentity == 'assign-bulk-test')) {
        filterByCondition = 3;
      } else {
  ...
  isCaseStudyTab = false;
  getActiveReporting(active, risStatusID, filterBy, formHeaderBGClass, formHeaderText) {
    this.isDoctorShare = false;
    this.isCaseStudyTab = formHeaderText === 'Case Study' ? true : false;
    this.TPId = null;
    // this.clearVariables(0);
    this.rowIndex = null;
    this.active = active;
  ...
  getTestSummaryInfo(activeTab) {
    switch (activeTab) {
      ...
      // }
      case 8: {
        this.testSummaryInfoHeader = this.screenIdentity === 'reporting-worklist' ? "Case Studies" : "Pended Worklist";
        break
      }
      case 9: {
  ...
  isWordSave = null;
  isWordSaved(event) {
    console.log('isWordsaved_', event)
    this.isWordSave = event
  }
  ...
  openPopupModal(row, index) {
    // console.log("risworklist row is_________",row)
    // this.clearVariables(0);
    this.rowIndex = null;
    // console.log("mo-row", row);
    this.rowIndex = index;
  ...
  refreshPagination() {
    ...
    if (this.screenIdentity == 'queue-management' || this.screenIdentity == 'assign-bulk-test') {
  ...
  isLoadByDefault = false;
  assignTest(row, index) {
    // console.log("Queue mgt row: ", row)
    this.isLoadByDefault = false;
    this.VisitId = null;
    this.isMetal = row.isMetal;
  ...
  getRadiologistInfo(EmpID) {
    ...
    this.questionnaireSrv.getRadiologistInfo(params).subscribe((res: any) => {
      if (res.StatusCode == 200) {
        ...
        this.EmpID = this.RadiologistID ? this.RadiologistID : this.radoiologistList[0].EmpId
  ...
  setAssignerLayout(assignerButtonSelected) {
    ...
    if ((this.screenIdentity == 'queue-management') && this.assignerButtonClicked == 1) {
      this.isAssigner = true;
    } else if (this.screenIdentity == 'queue-management' && this.assignerButtonClicked == 0) {
      this.isAssigner = false;
    }
    else {
      this.isAssigner == false;
  ...
  assignTestToDoctor(statusID, screen) {
    if (!this.EmpID && !this.TPIds.length && screen != 'bulk') {
      this.toastr.error("Please select the Doctor and  Test(s) to assign", "Validation Faild");
      return
    } else if (!this.EmpID && screen != 'bulk') {
      this.toastr.error("Please select Doctor", "Validation Faild");
      return
          ...
          if (data.StatusCode == 200) {
            if (this.withRemarks && statusID == 8 && respons[0].isExistTPStatus <= 8 && respons[0].isExistTPStatus > 0) {
              this.saveVisitRemarksAssigner({
                VisitId: Number(this.VisitID),
                ModuleName: 'Queue Management (Assigner)',
                Remarks: this.TPCode + " Assigned to " + this.AssigneeName,
                Priority: 1,
                UserId: this.loggedInUser.userid,
              })
            }
  ...
  assignTestToDoctorBulk(statusID) {
    ...
    if (isValidAssignmentObj) {
      this.toastr.error("Please select the doctor againt selected tests!", "Validation Failed");
      return;
    } else {
      this.clearRadiologistSummary();
      this.isDisabledAssignment = true;
      ...
      let objParam = {
        ...
        tblVisitTestAssignment: checkedItems.map(a => {
          ...
          return {
            ...
            VisitRemarks: (statusID == 8) ? a.TPCode + " assigned to " + assigneeName : a.TPCode + " unassigned from " + assigneeName,
            PRIORITY: 1
          }
        })
      }
      // return 
      // console.log("objParam for Assigner", objParam); //return;
      this.techSrv.insertUpdateBulkVisitTestAssignment(objParam).subscribe((data: any) => {
  ...
  openVisitDetailModal(row, i) {
    ...
    this.InitializedOn = row.InitializedOn;
    this.getVitals();
    if (this.screenIdentity == 'assign-bulk-test') {
      this.getRadiologistRefByMappingInfo(this.VisitID)
    }
  ...
  getEditExpiryTimeBadgeColor_(tempDSDateTime: string): string {
    // console.log ("tempDSDateTime bfore 15__",tempDSDateTime)
    // const updatedTime = moment(tempDSDateTime).add(15, 'minutes');
    // console.log ("tempDSDateTime after 15__",updatedTime)

    // // if (updatedTime <= tempDSDateTime) {
    // //   return 'badge badge-primary'; // Greater than or equal to 3 hours
    // // } 
    // // // else if (totalMinutes >= 0) {
    // // //   return 'badge badge-warning'; // Greater than or equal to 0 minutes
    ...
    const dbTime = moment(tempDSDateTime);

    // Add 15 minutes to the database time
    const dbTimePlus15Minutes = dbTime.add(15, 'minutes');

    // Get the current datetime
    const currentTime = moment();
    // Compare the adjusted database time with the current time
    if (dbTimePlus15Minutes.isSameOrBefore(currentTime)) {
      // Perform the first task if the database time + 15 minutes is <= current time
      return 'badge badge-danger'
    } else {
      // Perform the second task if the database time + 15 minutes is > current time
      return 'badge badge-primary'
    }

  }
  currentDateTime = null;
  getEditExpiryTimeBadgeColor__(tempDSDateTime: string): string {
    if (tempDSDateTime) {
      const dbTime = moment(tempDSDateTime);
      const dbTimePlus15Minutes = dbTime.add(15, 'minutes');
      const currentTime = moment();
      if (dbTimePlus15Minutes.isSameOrBefore(currentTime)) {
        ...
        // console.log('Result: badge-primary');
        return 'text-white badge badge-primary';
      }
    } else {
      return 'text-dark';
    }
  ...
  // with seconds:
  getEditExpiryTimeBadgeColor(tempDSDateTime: string): string {
    if (tempDSDateTime) {
      const dbTime = moment(tempDSDateTime).format('YYYY-MM-DDTHH:mm:ss');
      const dbTimePlus15Minutes = moment(dbTime).add(15, 'minutes');
      const currentTime = moment().format('YYYY-MM-DDTHH:mm:ss');
      if (moment(dbTimePlus15Minutes).isSameOrBefore(moment(currentTime))) {
        return 'text-white badge badge-danger';
      } else {
        // console.log('Result: badge-primary');
        return 'text-white badge badge-primary';
  ...
  // without seconds:
  getEditExpiryTimeBadgeColorWOSeconds(tempDSDateTime: string): string {
    if (tempDSDateTime) {
      const dbTime = moment(tempDSDateTime).format('YYYY-MM-DDTHH:mm');
      const dbTimePlus15Minutes = moment(dbTime).add(15, 'minutes');
  ...
  ////////////////////Doctor share services TS starts/////////////////////////
  ...
  isDoctorShare = false;
  getDoctorShareService(active, filterBy, formHeaderBGClass, formHeaderTex) {
    this.isDoctorShare = true;
    this.active = active;
    this.paramsValuesForWorkList.filterBy = filterBy;
    this.paramsValuesForWorkList.isActive = active;
    if (this.reportType == '2') {
      ...
      this.spinner.show(this.spinnerRefs.listSection);
      this.billingSrv.getRadioDocShareDetail(params).subscribe(
        (res: any) => {
          this.spinner.hide(this.spinnerRefs.listSection);
          if (res.StatusCode === 200) {
  ...
  getRadioShareDetails() {
    ...
    this.spinner.show(this.spinnerRefs.searchDoctorShareTable);
    this.billingSrv.getRadioDocShareDetail(params).subscribe((res: any) => {
      this.spinner.hide(this.spinnerRefs.searchDoctorShareTable);
      if (res.StatusCode === 200) {
        this.DoctorShareDetalList = res.PayLoad;
        this.ifDataPrint = true;
        this.groupDataByDoctorAndTest();
  ...
  getRadioShareSummary() {
    ...
    this.spinner.show(this.spinnerRefs.searchDoctorShareTable);
    this.billingSrv.getRadioDocShareSummary(params).subscribe((res: any) => {
      this.spinner.hide(this.spinnerRefs.searchDoctorShareTable);
      if (res.StatusCode === 200) {
  ...
  onChange(event: any) {
    ...
    this.getDoctorShareService(9, 10, 'bg-danger', 'Doctor share list');
  }
  ...
  closeReportingModal() {
    ...
    this.reportingModalRef.close()
  }

  confirmationPopoverConfigAI = {
    placements: ['top', 'left', 'right', 'bottom'],
    popoverTitle: 'Please Confirm...!', // 'Are you sure?',
    popoverMessage: 'Are you <b>sure</b> want to send the request to AI ?',
    confirmText: 'Yes <i class="fa fa-check"></i>',
    cancelText: 'No <i class="fa fa-times"></i>',
    confirmClicked: false,
    cancelClicked: false,
  ...
  requestAIAssistance(row, i) {
    this.rowIndex = i;
    const key = `${row.VisitNo}_${row.TPId}`;

    // Initialize state if missing
    if (!this.rowStates[key]) {
      this.rowStates[key] = { loading: false, requested: false };
    }

    // Prevent multiple clicks during request
    if (this.rowStates[key].loading) {
      return;
    }

    // Set loading state
    this.rowStates[key].loading = true;
    this.rowStates[key].requested = false;
    ...
    this.sharedService.insertUpdateData(API_ROUTES.REQUEST_AI_ASSISTANCE, dataObj).subscribe({
      next: (data: any) => {
        try {
          ...
          if (data.StatusCode === 200) {
            ...
            this.rowStates[key].requested = true;   // ✅ success → show tick
          } else {
            this.toastr.error(data.Message || 'AI request failed');
            this.rowStates[key].requested = false;  // ❌ failure → brain comes back
          }
        } catch (error) {
      ...
      error: (err) => {
        ...
        this.toastr.error('AI request failed due to network/server error');
        this.rowStates[key].requested = false;     // ❌ failure → brain comes back
      },
      complete: () => {
        this.rowStates[key].loading = false;       // 🔄 stop spinner
      }
    });
  }
  ...
  noAIRequestAllowed() {
    this.toastr.warning('Images are not available or you don’t have permission.');
  }

  getAiIcon(index: number): string {
    const basePath = './assets/images/brand/aiicons/';
    return this.rowIndex === index ? `${basePath}ai-white.png` : `${basePath}ai-green.png`;
  }
  getNotAllowedAiIcon(index: number): string {
    const basePath = './assets/images/brand/aiicons/';
    return this.rowIndex === index ? `${basePath}ai-white.png` : `${basePath}ai-gray.png`;
  }
