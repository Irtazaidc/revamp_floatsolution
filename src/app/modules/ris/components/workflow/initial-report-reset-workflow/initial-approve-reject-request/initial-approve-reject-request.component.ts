...
@Component({
  ...
  selector: 'app-initial-approve-reject-request',
  templateUrl: './initial-approve-reject-request.component.html',
  styleUrls: ['./initial-approve-reject-request.component.scss']
})
export class InitialApproveRejectRequestComponent implements OnInit {
  ...
  disabledButtonApproved = false;
  isSpinnerReject = true;
  isSpinnerApprove = true;
  isSpinnerRecommend = true;
  RejectApproveRequestRemarks = "";
  characterCount = 0;
  maxCharacterCount = 15;
  rowIndex = null;
  ResetRequestRemarks = null;
  ResetRequestRecommendationRemarks = null;
  ...
  updateCharacterCount() {
    this.characterCount = this.RejectApproveRequestRemarks.length;
  }
  ...
  getVisitTestReset() {
    ...
    this.sharedService.getData(API_ROUTES.GET_VISIT_TEST_RESET, params).subscribe((res: any) => {
      this.noDataMessage = 'No Recommended Request Found';
  ...
  getRowData(selectedRow, i) {
    ...
    this.ResetRequestRecommendationRemarks = selectedRow.RecRemarks;
    this.RequestedBy = selectedRow.RequestedBy;
    this.RequestedOn = selectedRow.RequestedOn;
    this.RecommendedBy = selectedRow.RecommendedBy;
    this.RecommendedOn = selectedRow.RecommendedOn;
  }
  clickSubmitBtn=false;
  updateVisitTestResetStatus(StatusID) {
    // recRejValue: 1: for recommend, 2: Reject 
    if (this.RejectApproveRequestRemarks.length < 15) {
      ...
      this.disabledButtonApproved = true;
      if (StatusID == 16) {
