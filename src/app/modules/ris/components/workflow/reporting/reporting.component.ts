...
export class ReportingComponent implements OnInit, OnChanges, OnDestroy {
  ...
  confirmationPopoverConfigPreliminary = {
    placements: ['top', 'left', 'right', 'bottom'],
    popoverTitle: 'Please Confirm...!', // 'Are you sure?',
    popoverMessage: 'Are you <b>sure</b> want to save <strong class="text-danger">Preliminary</strong> ?',
    confirmText: 'Yes <i class="fa fa-check"></i>',
    cancelText: 'No <i class="fa fa-times"></i>',
    confirmClicked: false,
    cancelClicked: false,
  ...
  ngOnInit(): void {
    ...
    this.EmpId = this.ParamsPayload.EmpId;
    ...
    // console.log("this.ParamsPayload", this.ParamsPayload);
  ...
  //     console.log("🚀 ~ ReportingComponent ~ this.sharedService.getData ~ resp:", resp)
  //     let _response = resp.PayLoad;
  //     this.TPDisclaimerRow = _response;
  //     if (this.TPDisclaimerRow.length) {
  //       this.DDisclaimerID = this.TPDisclaimerRow[0].DDisclaimerID;
  //       this.TPDisclaimerID = this.TPDisclaimerRow[0].TPDisclaimerID;
  //       this.VisitTPDisclaimerID = this.TPDisclaimerRow[0].VisitTPDisclaimerID;
  //       this.DisclaimerBody = this.TPDisclaimerRow[0].DisclaimerBody;
  //       this.DisclaimerBodyHTML = this.TPDisclaimerRow[0].DisclaimerBodyHTML;
  //     }
  //   }, (err) => {
  //     this.toastr.error('Connection error');
  //   })
  ...
  getTPDisclaimer() {
    ...
    this.sharedService.getData(API_ROUTES.GET_TP_DISCLAIMER, objParam).subscribe(
      (resp: any) => {
        const _response = resp.PayLoad;
        this.TPDisclaimerRow = _response;

        if (this.TPDisclaimerRow.length) {
          // Find the first disclaimer where VisitTPDisclaimerID is not null or undefined
          ...
          this.onDisclaimerChange(this.selectedDisclaimer);
        }
      },
      (err) => {
        this.toastr.error('Connection error');
  ...
  insertUpdateRadioReport(radioEditStatus, statusID, actionBtn, RISStatusID = null) {
    ...
    dataObj = {
      ...
      StatusID: statusID,// Preliminary save status =8, 9 for Final
      isReportDelay: (statusID === 9 && this.isReportDelay) ? 1 : 0,
      RISStatusID: (statusID == 9) ? 13 : RISStatusID,// (statusID==7)?10:null,// 10 = draft null = preliminary  , 13 for final
    ...
    // console.log("dataObj____________", dataObj);return;
    this.startSpinner();
    this.enableButton();
    if(statusID == 9){
    ...
    this.sharedService.insertUpdateData(API_ROUTES.INSERT_UPDATE_RADIO_REPORT, dataObj).subscribe((data: any) => {
      if(statusID == 9){
        this.disabledButtonFinalizePopup = false;
        this.isSpinnerFinalizePopup = true;
      }
      // console.log("PayloadStr: ", JSON.parse(data.PayLoadStr))
      const res = JSON.parse(data.PayLoadStr);
      if (data.StatusCode === 200) {
        // this.StatusID = statusID;
        // this.RISStatusID = RISStatusID;
        this.spinner.hide(this.spinnerRefs.editorSection);
        // console.log("VisitTPDisclaimerID in inserting disclaimer id: ", this.VisitTPDisclaimerID)
        ...
        if (res[0].Result == 2) {
          ...
          this.toastr.error('Study has already been unassigned', 'Unassigned study');
        }
        ...
        if (statusID == 9 && res[0].Result != 2 || this.isRepeat) {
          if (!this.isRepeat) {
            this.ViewReport('');
          }
          this.appPopupService.closeModal();
  ...
  insertUpdateVisitTPDisclaimer() {
    const objDisclaimer = {
      ...
      DisclaimerBody: null,//this.DisclaimerBody,
      DisclaimerBodyHTML: null,//this.DisclaimerBodyHTML,
    ...
    this.techSrv.updateVisitTPStatusForInitialization(objParam).subscribe((data: any) => {
      ...
      if (respData.length) {
        ...
        if (data.StatusCode == 200) {
          ...
          this.toastr.error(data.Message || 'Error occured while saving tech history!')
        }
      }
    }, (err) => {
      console.log(err);
  ...
  reportFinalizationProcess(radioEditStatus, statusID) {
  ...
  selectFeedback(param) {
    this.isFeedback = param;
    if (!this.isFeedback) {
      this.insertUpdateRadioReport(this.RadioEditStatus, this.StatusID, 3);
    } else {
      this.getDSQuestions();
    }
  }
  finalizeReport() {
    ...
    this.insertUpdateRadioReport(3, 9, 4, 13);
...
this.disabledButtonAIImages = false;
  ...
  getPACSServersV2(VID, TPID, purpose) {
    const VisitIDsObj = this.comparativeStudies.filter(row => row.checked);
    const VisitIDs = "";
    const TPIDs = "";
    this.SysInfo = this.auth.getSystemInfoFromStorage();
