...
@Component({
  ...
  selector: "app-ris-delay-summary-report",
  templateUrl: "./ris-delay-summary-report.component.html",
  styleUrls: ["./ris-delay-summary-report.component.scss"],
})
export class RisDelaySummaryReportComponent implements OnInit {
  dueDelaySummaryReportDataList = [];
  ...
  filterDelaySummaryForm: FormGroup = this.formBuilder.group(
    this.delaySummaryFields
  );
  ...
  ngOnInit(): void {
    ...
    setTimeout(() => {
      this.filterDelaySummaryForm.patchValue({
        fromDate: Conversions.getPreviousDateObject(),
  ...
  getdelaySummaryReportData() {
    const formValues = this.filterDelaySummaryForm.getRawValue();
    const dateFrom = formValues.fromDate;
    const dateTo = formValues.toDate;
    ...
    if (this.filterDelaySummaryForm.invalid) {
    ...
    const objParams = {
      DateFrom: formValues.fromDate ? Conversions.formatDateObject(formValues.fromDate)  : null,
      DateTo: formValues.toDate ? Conversions.formatDateObject(formValues.toDate) : null,
    ...
    this.labTats.getDueDelayReportSummaryForRIS(objParams).subscribe(
      (res: any) => {
        this.spinner.hide(this.spinnerRefs.delayreportTable);
        if (res.StatusCode == 200 && res.PayLoad.length) {
          this.dueDelaySummaryReportDataList = res.PayLoad;
          console.log("res:", res);
          this.filterResults();
        } else {
          this.toasrt.info("No Record Found");
          this.dueDelaySummaryReportDataList = [];
        }
      },
      (err) => {
        console.log(err);
  ...
  getLocationList() {
   ...
   onSelectAllDelaySummaryBranches() {
    this.filterDelaySummaryForm.patchValue({
      LocIDs: this.branchList.map((a) => a.LocId),
    });
  }
  onUnselectAllDelaySummaryBranches() {
    this.filterDelaySummaryForm.patchValue({
      LocIDs: [],
    });
  }
  ...
  exportAsExcel() {
    const excelData = [];
    if (this.dueDelaySummaryReportDataList.length) {
      this.dueDelaySummaryReportDataList.forEach((d, index) => {
        const row = {
          "Sr#": index + 1,
  ...
  filterResults() {
    ...
    const cols = ["SubSectionName", "DelayedReports", "TestCount"];

    let results: any = this.dueDelaySummaryReportDataList;
    ...
    if (this.searchText && this.searchText.length > 1) {
      ...
      results = this.dueDelaySummaryReportDataList.filter((item: any) => {
